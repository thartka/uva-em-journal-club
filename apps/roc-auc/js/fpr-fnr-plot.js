// Canvas: left y = FNR, right y = FPR, x = probability threshold; vertical line at threshold
...
class FPRFNRPlotRenderer {
    ...
    draw() {
        ...
        const toYLeft = (fnr) => this.padding.top + plotHeight - fnr * plotHeight;
        const toYRight = (fpr) => this.padding.top + plotHeight - fpr * plotHeight;
        const toY = toYLeft;
        ...
        if (this.curveData.length >= 2) {
            ...
            this.ctx.beginPath();
            this.ctx.moveTo(toX(this.curveData[0].threshold), toY(this.curveData[0].fnr));
            for (let i = 1; i < this.curveData.length; i++) {
                this.ctx.lineTo(toX(this.curveData[i].threshold), toY(this.curveData[i].fnr));
            }
            this.ctx.stroke();
            ...
            this.ctx.lineWidth = 2;
            this.ctx.beginPath();
            this.ctx.moveTo(toX(this.curveData[0].threshold), toY(this.curveData[0].fpr));
            for (let i = 1; i < this.curveData.length; i++) {
                this.ctx.lineTo(toX(this.curveData[i].threshold), toY(this.curveData[i].fpr));
            }
            this.ctx.stroke();
        }

        const vx = toX(this.currentThreshold);
        ...
        this.ctx.fillText('False Negative Rate', 0, 0);
        this.ctx.restore();

        this.ctx.save();
        this.ctx.translate(width - 18, this.padding.top + plotHeight / 2);
        this.ctx.rotate(Math.PI / 2);
        this.ctx.fillText('False Positive Rate', 0, 0);
        this.ctx.restore();
        ...
        this.ctx.textAlign = 'left';
        this.ctx.fillText('FNR', this.padding.left + plotWidth / 2 - 64, this.padding.top - 12);
        this.ctx.fillStyle = '#c62828';
        this.ctx.fillRect(this.padding.left + plotWidth / 2 - 10, this.padding.top - 18, 12, 3);
        this.ctx.fillStyle = '#333';
        this.ctx.fillText('FPR', this.padding.left + plotWidth / 2 + 6, this.padding.top - 12);
    }
}
