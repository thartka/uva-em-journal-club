...
const TypeErrorQuestions = (() => {
    ...
    const QUESTIONS = [
        ...
        },
        {
            ...
            correct: 'A',
            explanations: {
                A: 'Correct. A Type II error is failing to reject a false null — missing an effect that is truly there (a false negative). Its probability is β, and power = 1 − β.',
                B: 'A Type I error is a false positive — claiming an effect that is not real. Here a real effect was missed.',
        ...
        },
        {
            ...
            options: [
                { key: 'A', text: 'Increasing the sample size' },
                { key: 'B', text: 'Lowering α to 0.01' },
                { key: 'C', text: 'Reducing the sample size' },
                { key: 'D', text: 'Choosing a more variable outcome' }
            ],
            correct: 'A',
            explanations: {
                A: 'Correct. A larger sample narrows the sampling distributions, so a true effect is easier to detect — power rises and β falls. A larger true effect and a less variable outcome also raise power.',
                B: 'Lowering α makes the test more stringent and <em>decreases</em> power (increases β) — the opposite of the goal.',
                C: 'Reducing the sample size decreases power and increases the risk of a Type II error.',
                D: 'Greater outcome variability decreases power; you would need an even larger sample to overcome it.'
            }
        },
        {
            id: 'q5',
            ...
            options: [
                { key: 'A', text: 'A clinically trivial difference reached statistical significance' },
                { key: 'B', text: 'The trial is almost certainly underpowered' },
                { key: 'C', text: 'This most likely represents a Type I error' },
                { key: 'D', text: 'The measured effect is too large to be plausible' }
            ],
            correct: 'A',
            explanations: {
                A: 'Correct. With a very large sample, even a 2-minute difference — clinically negligible — can be statistically significant. Statistical significance is not the same as clinical importance; judge the effect size and its confidence interval, not the p-value alone.',
                B: 'A trial of 200,000 patients that reaches significance is over-powered, not underpowered.',
                C: 'A Type I error is always possible, but the central problem here is that a real yet trivial effect was detected and risks being over-interpreted.',
